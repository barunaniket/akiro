...
pub struct SandboxConfig {
    ...
    /// Per-execution jail identifier. Generated by the PARENT (in `Sandbox::execute`) so it
    /// can deterministically clean up the child's `/tmp/judge_root_<jail_id>` directory after
    /// the child exits — `FsIsolation`'s Drop never runs because `execve` replaces the child
    /// image. Empty by default; the child falls back to a fresh UUID if it is ever unset.
    pub jail_id: String,
}
...
impl SandboxConfig {
    ...
    pub fn with_jail_id(mut self, jail_id: String) -> Self {
